  ...
  const categories = [
    {
      name: "Infrastructure",
      icon: (
        <Highway responsiveSizing="w-[6rem] h-[6rem] sm:w-[5rem] sm:h-[5rem]" />
      ),
    },
    {
      name: "Property",
      icon: (
        <House responsiveSizing="w-[6rem] h-[6rem] sm:w-[5rem] sm:h-[5rem]" />
      ),
    },
    {
      name: "Equity",
      icon: (
        <Briefcase responsiveSizing="w-[6rem] h-[6rem] sm:w-[5rem] sm:h-[5rem]" />
      ),
    },
    {
      name: "Alternatives",
      icon: (
        <Puzzle responsiveSizing="w-[6rem] h-[6rem] sm:w-[5rem] sm:h-[5rem]" />
      ),
    },
  ];
      ...
      <div className="grid grid-cols-2 w-full px-3 max-w-xl pt-6 gap-2 gap-y-[2rem] justify-items-center">
        {categories.map((cat, index) => (
          <motion.div
            ...
            key={index}
            className="bg-white rounded-3xl p-6 pt-10 shadow-md text-center w-full max-w-[15rem] relative hover:shadow-xl cursor-pointer select-none"
          >
            <motion.svg
              version="1.1"
              id="fi_471662"
            ...
            <h2 className="text-xs sm:text-sm font-medium mb-2">{cat.name}</h2>
